...
{#
    Staging model for Pinterest Ads daily metrics.
...
with raw_pinterest_ads as (
    ...
    from {{ source('airbyte_raw', '_airbyte_raw_pinterest_ads_insights') }}
),
...
pinterest_ads_extracted as (
    select
        ...
        raw.ad_data->>'total_conversion_value' as conversion_value_raw,
...
pinterest_ads_normalized as (
    select
        ...
        -- Spend (Pinterest can provide spend_in_micro_dollar or spend)
        case
            when spend_micro_raw is not null and trim(spend_micro_raw) != ''
                and trim(spend_micro_raw) ~ '^-?[0-9]+$' then
                greatest((trim(spend_micro_raw)::bigint / 1000000.0)::numeric, 0)
            when spend_raw is not null and trim(spend_raw) != ''
                and trim(spend_raw) ~ '^-?[0-9]+\.?[0-9]*([eE][+-]?[0-9]+)?$' then
        ...
        -- Campaign info
        campaign_name,
        coalesce(objective_type, 'awareness') as campaign_objective,

        -- Source identification
        'pinterest_ads' as source,

        -- Platform channel
        coalesce(objective_type, 'awareness') as platform_channel,
...
{{ incremental_filter('report_date', 'pinterest_ads') }}
