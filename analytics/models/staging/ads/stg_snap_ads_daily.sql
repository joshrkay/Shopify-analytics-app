{{
    config(
        ...
        schema='staging',
        unique_key=['tenant_id', 'report_date', 'platform_account_id', 'platform_campaign_id', 'ad_squad_id', 'ad_id'],
        incremental_strategy='delete+insert'
    )
}}

{#
    Staging model for Snapchat Ads daily metrics.
...
with raw_snap_ads as (
    ...
    from {{ source('airbyte_raw', '_airbyte_raw_snap_ads_insights') }}
),
...
snap_ads_extracted as (
    select
        ...
        raw.ad_data->>'start_time' as start_time_raw,
        raw.ad_data->>'spend' as spend_raw,
        raw.ad_data->>'impressions' as impressions_raw,
        raw.ad_data->>'swipes' as clicks_raw,
...
snap_ads_normalized as (
    select
        ...
        -- Date field (Snap uses start_time as timestamp)
        case
            when start_time_raw is null or trim(start_time_raw) = '' then null
            when start_time_raw ~ '^\d{4}-\d{2}-\d{2}'
                then start_time_raw::date
            else null
        end as report_date,

        -- Spend (Snap reports in micro-currency, divide by 1,000,000)
        case
            when spend_raw is null or trim(spend_raw) = '' then 0.0
            when trim(spend_raw) ~ '^-?[0-9]+$' then
                greatest((trim(spend_raw)::bigint / 1000000.0)::numeric, 0)
            when trim(spend_raw) ~ '^-?[0-9]+\.?[0-9]*([eE][+-]?[0-9]+)?$' then
                greatest(trim(spend_raw)::numeric, 0)
            else 0.0
        end as spend,
        ...
        -- Campaign info
        campaign_name,
        coalesce(objective, 'awareness') as campaign_objective,

        -- Source identification
        'snap_ads' as source,

        -- Platform channel
        coalesce(objective, 'awareness') as platform_channel,
...
{{ incremental_filter('report_date', 'snap_ads') }}
