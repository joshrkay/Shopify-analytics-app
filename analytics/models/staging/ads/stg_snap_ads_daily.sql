{{
    config(
        ...
        enabled=var('enable_snap_ads', true)
    )
}}

{#
    Staging model for Snapchat Ads daily performance.

    Snapchat uses ad_account_id and reports in micros (1/1,000,000).
    Supports Story Ads, Collection Ads, and Spotlight placements.
#}
...
with raw_snap_ads as (
    ...
    from {{ source('raw_sources', 'raw_snap_ads_metrics') }}
    {% if is_incremental() %}
    where {{ incremental_timestamp_filter('_airbyte_emitted_at', 'snap_ads') }}
    {% endif %}
),
...
snap_ads_extracted as (
    select
        ...
        raw.ad_data->>'ad_squad_id' as ad_squad_id_raw,  -- Snap calls ad groups "ad squads"
        ...
        raw.ad_data->>'swipes' as clicks_raw,  -- Snap calls clicks "swipes"
...
snap_ads_normalized as (
    select
        ...
        campaign_name,
        ad_squad_name as adgroup_name,
        ad_name,
        objective,

        coalesce(lower(trim(placement)), 'snap_ads') as platform_channel,
        ...
        -- Snap reports spend in micros
        case
            when spend_raw is null or trim(spend_raw) = '' then 0.0
            when trim(spend_raw) ~ '^-?[0-9]+$'
                then greatest(trim(spend_raw)::bigint / 1000000.0, 0)
            else 0.0
        end as spend,
...
final as (
    select
        ...
        platform_channel,
        {{ map_canonical_channel("'snap_ads'", 'platform_channel') }} as canonical_channel,

        platform_account_id,
        {{ generate_internal_account_id('tenant_id', "'snap_ads'", 'platform_account_id') }} as internal_account_id,

        platform_campaign_id,
        {{ generate_internal_campaign_id('tenant_id', "'snap_ads'", 'platform_campaign_id') }} as internal_campaign_id,

        platform_adgroup_id,
        {{ generate_internal_adgroup_id('tenant_id', "'snap_ads'", 'platform_adgroup_id') }} as internal_adgroup_id,

        platform_ad_id,
        {{ generate_internal_ad_id('tenant_id', "'snap_ads'", 'platform_ad_id') }} as internal_ad_id,
