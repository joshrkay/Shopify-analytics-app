{{
    config(
        ...
        schema='staging',
        unique_key=['tenant_id', 'report_date', 'platform_account_id', 'platform_campaign_id', 'adgroup_id', 'ad_id'],
        incremental_strategy='delete+insert'
    )
}}

{#
    Staging model for TikTok Ads daily metrics.
...
with raw_tiktok_ads as (
    ...
    from {{ source('airbyte_raw', '_airbyte_raw_tiktok_ads_metrics') }}
),
...
tiktok_ads_extracted as (
    select
        ...
        raw.ad_data->>'stat_time_day' as stat_time_day_raw,
        ...
        raw.ad_data->>'complete_payment_events' as complete_payment_raw,
        ...
        raw.ad_data->>'objective_type' as objective_type
...
tiktok_ads_normalized as (
    select
        ...
        -- Date field
        case
            when stat_time_day_raw is null or trim(stat_time_day_raw) = '' then null
            when stat_time_day_raw ~ '^\d{4}-\d{2}-\d{2}'
                then stat_time_day_raw::date
            else null
        end as report_date,
        ...
        -- Conversions (TikTok uses 'conversion' or 'complete_payment_events')
        case
            when conversions_raw is null or trim(conversions_raw) = '' then
                case
                    when complete_payment_raw is null or trim(complete_payment_raw) = '' then 0.0
                    when trim(complete_payment_raw) ~ '^-?[0-9]+\.?[0-9]*$'
                        then greatest(trim(complete_payment_raw)::numeric, 0)
                    else 0.0
                end
        ...
        -- Campaign info
        campaign_name,
        coalesce(objective_type, 'traffic') as campaign_objective,

        -- Source identification
        'tiktok_ads' as source,

        -- Platform channel
        coalesce(objective_type, 'traffic') as platform_channel,
...
{{ incremental_filter('report_date', 'tiktok_ads') }}
