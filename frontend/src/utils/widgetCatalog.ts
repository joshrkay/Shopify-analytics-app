...
import type { ReportTemplate, WidgetCatalogItem, WidgetSize, ChartType, ChartConfig } from '../types/customDashboards';
...
export function extractWidgetCatalogItems(
  ...
  return template.reports_json.map((report: Record<string, unknown>, index: number) => {
    const chartType = report.chart_type as ChartType;
    ...
    const name = (report.name as string) || `Widget ${index + 1}`;
    ...
    return {
      ...
      name,
      title: name,
      description: (report.description as string) || template.description,
      category: chartType,
      businessCategory: mapChartTypeToWidgetCategory(chartType),
      chart_type: chartType,
      thumbnail_url: template.thumbnail_url ?? undefined,
      default_config: report.config_json as ChartConfig,
      required_dataset: report.dataset_name as string,
      defaultSize,
    };
  });
}
