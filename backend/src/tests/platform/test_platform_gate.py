...
def create_mock_authz_result(is_authorized=True, roles=None, tenant_id=None, user_id=None):
    ...
    Note: roles defaults to None (not ['admin', 'user']) so that the JWT roles
    are used in the tenant context instead of being overridden by the mock.
    This allows proper RBAC testing where different users have different roles.
    """
